...
(function (window, undefined) {
  ...
  let cb
  const callbacks = ['tick', 'second', 'finish']
 ...
 */
  const Clock = function (callbacks) {
 ...
 */
  for (let i = 0; i < 3; i++) {
    ...
    Clock.prototype['call_' + cb] = (function (callbackType) {
      return function () {
        try {
          if (this.callbacks[callbackType] !== undefined) { this.callbacks[callbackType](this) }
        } catch (ex) {
 ...
 */
  Clock.prototype.countdown = function () {
    let whole_minutes; const elapsed_seconds = Math.floor((new Date().getTime() - this.start_time) / 1000)
